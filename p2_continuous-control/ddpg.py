...
class DDPGAgent:
    def __init__(self, input_size: int, action_size: int, hidden_size: int, seed: int, eps: float, device):
        ...
        self.critic_target = CriticNetwork(input_size, action_size, hidden_size)
        self.critic_local = CriticNetwork(input_size, action_size, hidden_size)
    ...
    def learn(self, experiences: Tuple, gamma: float):
        ...
        print(critic_loss, target_q[0], current_q[0])
        actions_for_policy = self.actor_local(states)
        q_for_policy = self.critic_local(states, actions_for_policy).squeeze(1)
        policy_loss = -torch.mean(q_for_policy)
        self.actor_optimizer.zero_grad()
        policy_loss.backward()
        self.actor_optimizer.step()


def ddpg(agent: DDPGAgent, env, brain_name, n_episodes: int = 200, max_t: int = 1000, eps_end=0.1, eps_decay=0.995):
    ...
    torch.save(agent.critic_local.state_dict(), 'ddpg_critic_checkpoint.pth')
    torch.save(agent.actor_local.state_dict(), 'ddpg_actor_checkpoint.pth')
